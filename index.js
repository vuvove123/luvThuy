const contentLetterSrart_actived = "Gửi đôi lời tới chị iu <3" //Lời mở đầu cho bức thư
fetch('./txt/Baitap18.txt')
    .then(response => response.text())
    .then(text => {
        // Lưu nội dung vào biến mainContentLetter
        mainContentLetter = text;
        // Gọi hàm hoặc thực hiện các thao tác khác với nội dung ở đây
        processText(mainContentLetter);
    })
    .catch(error => console.error(error));

// Hàm xử lý nội dung văn bản
function processText(text) {
    console.log(text); // Ví dụ: in nội dung văn bản ra console
    // Thực hiện các thao tác khác với nội dung văn bản ở đây
    document.getElementById('text-content').innerHTML = '<pre>' + text + '</pre>';

}
let mainContentLetter = './txt/Baitap18.txt'//Nội dung của bức thư
